...
class ModelSellerShippingShipping extends Model {
	public function getQuote($address,$shipping,$seller_id,$product_id) {
		...
		if($shipping == 'free'){
			...
			if (!$this->config->get($seller_id.':free_geo_zone_id')) {
				$status = true;
			} elseif ($query->num_rows) {
				$status = true;
			} else {
			...
			if ($this->cart->getSubTotal() < $this->config->get($seller_id.':free_total')) {
				$status = false;
			}
			...
			if ($status) {
				...
				$method_data = array(
					'code'       => 'free',
					'title'      => $this->language->get($seller_id.':text_title'),
					'quote'      => $quote_data,
					'sort_order' => $this->config->get($seller_id.':free_sort_order'),
					'error'      => false
				);
			}
		...
		if($shipping == 'item'){
			...
			if (!$this->config->get($seller_id.':item_geo_zone_id')) {
				$status = true;
			} elseif ($query->num_rows) {
				$status = true;
			} else {
			...
			if ($status) {
				...
				foreach ($this->model_sellerproduct_product->getProductsOnCartBySellerId($seller_id) as $product) {
					if ($product['shipping']) {
				...
				$quote_data['item'] = array(
					'code'         => $seller_id.':item.item',
					'title'        => $this->language->get('text_description'),
					'cost'         => $this->config->get($seller_id.':item_cost') * $items,
					'tax_class_id' => $this->config->get($seller_id.':item_tax_class_id'),
					'text'         => $this->currency->format($this->tax->calculate($this->config->get($seller_id.':item_cost') * $items, $this->config->get($seller_id.':item_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
				);
				...
				$method_data = array(
					...
					'title'      => $this->language->get($seller_id.':text_title'),
					'quote'      => $quote_data,
					'sort_order' => $this->config->get($seller_id.':item_sort_order'),
					'error'      => false
				);
			}
