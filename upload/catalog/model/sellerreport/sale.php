<?php
class ModelSellerReportSale extends Model {
	...
	public function getSellerTotalOrdersByCountry() {


$query = $this->db->query("
SELECT *,COUNT(*) AS total, SUM(op.total) AS amount, SUM(ot.value) AS seller_shipping_total, c.iso_code_2
...
LEFT JOIN `" . DB_PREFIX . "product_to_seller` cp ON (cp.product_id = op.product_id)
LEFT JOIN `" . DB_PREFIX . "order_total` ot ON (ot.order_id = o.order_id)
WHERE cp.seller_id =".$this->customer->getId()."
AND ot.seller_id =".$this->customer->getId()."
AND o.order_status_id > '0' GROUP BY o.payment_country_id ");
